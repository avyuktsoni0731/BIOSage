...
import { ChevronRight, AlertCircle, Info, Cpu, MemoryStickIcon as Memory, HardDrive, Thermometer } from "lucide-react"
...
export default function BiosSimulator() {
    ...
    if (bootProgress < 100) {
      const timer = setTimeout(() => {
        setBootProgress((prev) => {
          ...
          if (newProgress === 65) {
            setErrorMessage("ERROR: Memory module in DIMM_A2 reporting inconsistent timings")
          }
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      switch (e.key) {
        case "Tab":
          ...
          if (e.shiftKey) {
            setActiveTab((prev) =>
              prev === "dashboard" ? "diagnostics" : prev === "diagnostics" ? "hardware" : "dashboard",
            )
          } else {
            setActiveTab((prev) =>
              prev === "dashboard" ? "hardware" : prev === "hardware" ? "diagnostics" : "dashboard",
            )
          }
          break
        case "ArrowUp":
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-black text-gray-200 p-4 font-mono" tabIndex={0}>
      {/* Header */}
      <div className="flex justify-between items-center mb-4">
        <div>
          <h1 className="text-xl font-bold text-white">ADVANCED BIOS INTERFACE v2.0</h1>
          <p className="text-xs text-gray-400">HACKATHON EDITION</p>
        </div>
            ...
            <div className="flex-1">
              ...
              {!diagnosticResult && (
                <p className="text-xs text-gray-400 mt-1">
                  Press <kbd className="px-1 bg-gray-800 rounded">F1</kbd> to analyze error
                </p>
              )}
            </div>
          </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        <TabsList className="bg-gray-900 border border-gray-800">
          <TabsTrigger value="dashboard" className="data-[state=active]:bg-gray-800 data-[state=active]:text-white">
            System Dashboard
          </TabsTrigger>
          <TabsTrigger value="hardware" className="data-[state=active]:bg-gray-800 data-[state=active]:text-white">
            Hardware Monitor
          </TabsTrigger>
          <TabsTrigger value="diagnostics" className="data-[state=active]:bg-gray-800 data-[state=active]:text-white">
            Diagnostics
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="dashboard" className="space-y-4">
          ...
          <Card className="border-gray-800 bg-gray-900/50 p-4">
            ...
            <div className="bg-black/50 p-3 rounded text-sm font-mono h-40 overflow-y-auto">
              <p className="text-green-400">[00:00:01] Initializing system hardware</p>
              <p className="text-green-400">[00:00:02] CPU: AMD Ryzen 9 7950X detected</p>
              <p className="text-green-400">[00:00:03] Memory: 32GB DDR5-6000 detected</p>
              <p className="text-green-400">[00:00:04] Storage: 2TB NVMe SSD detected</p>
              <p className="text-green-400">[00:00:05] Loading BIOS modules</p>
              <p className="text-green-400">[00:00:07] Initializing PCIe devices</p>
              <p className="text-green-400">[00:00:09] GPU: NVIDIA RTX 4090 detected</p>
              <p className="text-green-400">[00:00:11] Network: Intel AX211 detected</p>
              <p className="text-green-400">[00:00:13] Running memory tests</p>
              <p className="text-red-400">[00:00:15] ERROR: Memory module in DIMM_A2 reporting inconsistent timings</p>
              <p className="text-yellow-400">[00:00:16] WARNING: Memory running at reduced speed</p>
              <p className="text-green-400">[00:00:18] Storage detection complete</p>
              <p className="text-green-400">[00:00:20] Waiting for boot device selection</p>
            </div>
          </Card>
        </TabsContent>
              ...
              <div className="mt-4">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <Badge variant="outline" className="bg-yellow-950/30 text-yellow-400 border-yellow-800">
                      Warning
                    </Badge>
                  </div>
                </div>
        ...
        <TabsContent value="diagnostics" className="space-y-4">
          <Card className="border-gray-800 bg-gray-900/50 p-4">
            ...
            <div className="space-y-2">
              <div className={`flex items-center p-2 ${selectedItem === 0 ? "bg-gray-800" : ""}`}>
                <ChevronRight size={16} className={selectedItem === 0 ? "text-white" : "text-gray-500"} />
                <span className="ml-2">Run Memory Test</span>
              </div>

              <div className={`flex items-center p-2 ${selectedItem === 1 ? "bg-gray-800" : ""}`}>
                <ChevronRight size={16} className={selectedItem === 1 ? "text-white" : "text-gray-500"} />
                <span className="ml-2">Run CPU Stress Test</span>
              </div>

              <div className={`flex items-center p-2 ${selectedItem === 2 ? "bg-gray-800" : ""}`}>
                <ChevronRight size={16} className={selectedItem === 2 ? "text-white" : "text-gray-500"} />
                <span className="ml-2">Check Storage Health</span>
              </div>

              <div className={`flex items-center p-2 ${selectedItem === 3 ? "bg-gray-800" : ""}`}>
                <ChevronRight size={16} className={selectedItem === 3 ? "text-white" : "text-gray-500"} />
                <span className="ml-2">Analyze Boot Performance</span>
              </div>

              <div className={`flex items-center p-2 ${selectedItem === 4 ? "bg-gray-800" : ""}`}>
                <ChevronRight size={16} className={selectedItem === 4 ? "text-white" : "text-gray-500"} />
                <span className="ml-2">Verify System Integrity</span>
              </div>

              <div className={`flex items-center p-2 ${selectedItem === 5 ? "bg-gray-800" : ""}`}>
                <ChevronRight size={16} className={selectedItem === 5 ? "text-white" : "text-gray-500"} />
                <span className="ml-2">Export Diagnostic Report</span>
              </div>
            </div>

            <div className="mt-4 text-xs text-gray-400">
              Use <kbd className="px-1 bg-gray-800 rounded">↑/↓</kbd> to navigate and{" "}
              <kbd className="px-1 bg-gray-800 rounded">Enter</kbd> to select
            </div>
          </Card>
          ...
          <Card className="border-gray-800 bg-gray-900/50 p-4">
            ...
            <div className="bg-black/50 p-3 rounded text-sm font-mono h-40 overflow-y-auto">
              <p className="text-red-400">
                [05/31/2025 15:45:22] ERROR: Memory module in DIMM_A2 reporting inconsistent timings
              </p>
              <p className="text-yellow-400">[05/31/2025 15:45:23] WARNING: Memory running at reduced speed</p>
              <p className="text-yellow-400">
                [05/30/2025 09:12:15] WARNING: HDD S.M.A.R.T. status degrading (Reallocated Sectors Count: 24)
              </p>
              <p className="text-red-400">[05/29/2025 22:34:01] ERROR: CPU temperature exceeded threshold (95°C)</p>
              <p className="text-yellow-400">
                [05/29/2025 22:34:05] WARNING: CPU throttling enabled due to thermal limits
              </p>
              <p className="text-red-400">[05/28/2025 14:22:33] ERROR: PCIe device in slot 2 failed to initialize</p>
              <p className="text-yellow-400">[05/27/2025 08:15:42] WARNING: CMOS battery low</p>
            </div>
          </Card>
        </TabsContent>
      </Tabs>
      ...
      <div className="mt-4 border-t border-gray-800 pt-2 text-xs text-gray-500 flex justify-between">
        <div>BIOS v2.0.5 | Build Date: 05/31/2025</div>
        <div>Hackathon BIOS Simulator</div>
      </div>
    </div>
  )
}
