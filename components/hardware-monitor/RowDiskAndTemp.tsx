...
export default function RowDiskAndTemp({ metrics, diskData, tempData }: RowDiskAndTempProps) {
  return (
      ...
      {metrics.disk.readSpeed !== null && metrics.disk.writeSpeed !== null && (
        <Card className="border-gray-800 bg-gray-900/50 p-4">
          ...
          <div className="mt-2 flex justify-between text-sm">
            <span className="text-orange-400">Read: {metrics.disk.readSpeed} MB/s</span>
            <span className="text-orange-300">Write: {metrics.disk.writeSpeed} MB/s</span>
          </div>
        </Card>
      )}
          ...
          <div className="h-40">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={tempData}>
                <XAxis dataKey="time" tick={false} axisLine={{ stroke: 'transparent' }} />
                <YAxis domain={[20, 100]} tick={false} axisLine={{ stroke: 'transparent' }} />
                <Tooltip
                  content={({ active, payload }) => {
                    if (active && payload && payload.length) {
                      return (
